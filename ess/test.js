...
function getGajiPokokByYear(tahun) {
  const years = Object.keys(gajiPokokPerTahun).map(Number).sort((a,b)=>a-b);
  for (let i = years.length - 1; i >= 0; i--) {
    if (tahun >= years[i]) return gajiPokokPerTahun[years[i]];
  }
  return gajiPokokPerTahun[years[0]];
}

function getGajiConstByDate(tanggalStr) {
  const d = parseIndoDate(tanggalStr);
  const tahun = d ? d.getFullYear() : new Date().getFullYear();
  const GAJI_POKOK = getGajiPokokByYear(tahun);

  return {
    GAJI_POKOK,
    DAILY_RATE: GAJI_POKOK / 25,
    OVERTIME_RATE: GAJI_POKOK / 173,
    MEAL_RATE: 11800,
    POTONGAN: (GAJI_POKOK * 0.02) + (GAJI_POKOK * 0.01)
  };
}
...
function calculateStats(data) {
  ...
  const refTanggal = data[0]?.["Tanggal"];
  const g = getGajiConstByDate(refTanggal);

  const BPJS = hariKerja > 0 ? g.POTONGAN : 0;
  const gajiPokokFinal = (data.length >= 30) ? g.GAJI_POKOK : (hariKerja * g.DAILY_RATE);

  const gaji =
    gajiPokokFinal +
    (totalIndeks * g.OVERTIME_RATE) +
    (meal * g.MEAL_RATE) -
    BPJS;
